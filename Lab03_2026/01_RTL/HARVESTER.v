...
always @(posedge clk or negedge rst_n) begin
    if (~rst_n) begin
        ...
        // 原本在沒有 rst_n 的 always block 中的變數 (如 alu_in_l_reg 等)
        // 刻意不在此處給予 reset 初始值，以維持原本的資料路徑行為。
    end 
    else begin
        // ==========================================
        // Pipeline Stages
        ...
        if (cs == CALC_ALU_EXEC) begin
            alu_out_reg <= alu_out_w; 
        end

        // ==========================================
        // Stack Push & Update & Control Path
        // ==========================================
        if (start) begin 
            src_row_reg      <= in_src_row; 
